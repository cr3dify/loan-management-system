...
-- 员工盈亏：所有用户可查看（简化版）
CREATE POLICY "Users can view profits" ON public.employee_profits
    FOR SELECT USING (true);

-- 审批记录：所有用户可管理（简化版）
CREATE POLICY "Users can manage approvals" ON public.approval_records
    FOR ALL USING (true);
