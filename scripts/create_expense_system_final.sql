...
-- 员工盈亏：所有用户可查看（临时策略）
CREATE POLICY "Anyone can view profits" ON public.employee_profits
    FOR SELECT USING (true);

-- 审批记录：所有用户可管理（临时策略）
CREATE POLICY "Anyone can manage approvals" ON public.approval_records
    FOR ALL USING (true);
...
BEGIN
    ...
    IF NOT FOUND THEN
        RETURN json_build_object('success', false, 'message', '费用记录不存在');
    END IF;
    ...
    SET 
        ...
        approved_at = NOW(),
        rejection_reason = CASE WHEN p_approval_status = 'rejected' THEN p_comments ELSE NULL END,
        updated_at = NOW()
    WHERE id = p_expense_id;

    -- 创建审批记录
    INSERT INTO public.approval_records (
        record_type, record_id, approver_id,
        approval_status, approval_level, comments
    ) VALUES (
        'expense', p_expense_id, p_approver_id,
        p_approval_status, 1, p_comments
    );
